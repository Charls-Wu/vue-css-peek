...
import { createConnection, TextDocuments, ProposedFeatures, TextDocumentSyncKind, TextDocumentPositionParams, Definition, InitializeParams, TextDocument } from 'vscode-languageserver';
import { StylesheetMap, Selector } from './types';
...
import { getLanguageService as getHTMLLanguageService, Node } from 'vscode-html-languageservice';
...
function searchStyleTag(roots: Node[]): Node {
...
connection.onDefinition(
  (textDocumentPositon: TextDocumentPositionParams): Definition => {
    ...
    if (document.languageId === 'vue') {
      ...
      // 先分析出来style的位置
      const HDocument = TextDocument.create(uri, 'html', 1, text);
      const languageService = getHTMLLanguageService();
...
connection.onWorkspaceSymbol(({ query }) => {
  ...
  return selectors.reduce((p, selector) => [...p, ...findSymbols(selector, styleSheets)], []);
});
