...
export function findSymbols(selector: Selector, stylesheetMap: StylesheetMap): SymbolInformation[] {
  ...
  Object.keys(stylesheetMap).forEach(uri => {
    const { document, stylesheet, startRow } = stylesheetMap[uri];
    try {
      ...
      symbols.forEach((symbol: SymbolInformation) => {
        ...
        if (symbol.name.search(re) !== -1) {
          symbol.location.range.start.line = symbol.location.range.start.line + startRow;
          symbol.location.range.end.line = symbol.location.range.end.line + startRow;
